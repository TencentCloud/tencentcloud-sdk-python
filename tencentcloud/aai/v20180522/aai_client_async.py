...
class AaiClient(AbstractClient):
    ...
    async def Chat(
        ...
        kwargs["resp_cls"] = models.ChatResponse
    ...
    async def SimultaneousInterpreting(
            self,
            request: models.SimultaneousInterpretingRequest,
            opts: Dict = None,
    ) -> models.SimultaneousInterpretingResponse:
        """
        该接口是实时流式识别，可同时返回语音识别文本及翻译文本，当前仅支持中文和英文。该接口可配合同传windows客户端，提供会议现场同传服务。
        """
        
        kwargs = {}
        kwargs["action"] = "SimultaneousInterpreting"
        kwargs["params"] = request._serialize()
        kwargs["resp_cls"] = models.SimultaneousInterpretingResponse
        kwargs["headers"] = request.headers
        kwargs["opts"] = opts or {}
