...
class CvmClient(AbstractClient):
    ...
    def ModifyChcNetworkMode(self, request):
        r"""ModifyChcNetworkMode接口用于切换CHC物理服务器的网络模式，适用于客户使用自建pxe环境装机，调用此接口切换部署网络和业务网络。**调用此接口会影响到业务网络，请明确使用方法后再调用**。
        ...
        :param request: Request instance for ModifyChcNetworkMode.
        :type request: :class:`tencentcloud.cvm.v20170312.models.ModifyChcNetworkModeRequest`
        :rtype: :class:`tencentcloud.cvm.v20170312.models.ModifyChcNetworkModeResponse`

        """
        try:
            params = request._serialize()
            headers = request.headers
            body = self.call("ModifyChcNetworkMode", params, headers=headers)
            response = json.loads(body)
            model = models.ModifyChcNetworkModeResponse()
            model._deserialize(response["Response"])
            return model
        except Exception as e:
            if isinstance(e, TencentCloudSDKException):
