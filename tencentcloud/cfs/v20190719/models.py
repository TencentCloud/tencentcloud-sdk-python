...
class LifecycleDataTaskInfo(AbstractModel):
    ...
    @property
    def TaskStatus(self):
        r"""<p>任务状态</p><p>枚举值：</p><ul><li>init： 排队中</li><li>running： 执行中</li><li>finished： 已完成</li><li>failed： 失败</li><li>stopping： 停止中</li><li>stopped： 已停止</li></ul>
        :rtype: str
        """
...
class SetUserQuotaRequest(AbstractModel):
    ...
    def FileHardLimit(self):
        r"""<p>文件硬限制</p><p>取值范围：[1000, 1000000000]</p><p>单位：个</p><p>默认值：无默认值</p>
        :rtype: int
        """
