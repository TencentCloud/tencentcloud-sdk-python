...
class EcmClient(AbstractClient):
    ...
    def AssignIpv6Addresses(self, request):
        r"""本接口（AssignIpv6Addresses）用于弹性网卡申请IPv6地址。
    ...
    def AssignIpv6CidrBlocks(self, request):
        r"""本接口（AssignIpv6CidrBlocks）用于分配IPv6网段。
    ...
    def AssociateAddress(self, request):
        r"""将弹性公网IP（简称 EIP）绑定到实例或弹性网卡的指定内网 IP 上。
    ...
    def CreateKeyPair(self, request):
        r"""用于创建一个 OpenSSH RSA 密钥对，可以用于登录 Linux 实例。
    ...
    def CreateRouteTable(self, request):
        r"""创建了VPC后，系统会创建一个默认路由表，所有新建的子网都会关联到默认路由表。默认情况下您可以直接使用默认路由表来管理您的路由策略。当您的路由策略较多时，您可以调用创建路由表接口创建更多路由表管理您的路由策略。
    ...
    def CreateSecurityGroupPolicies(self, request):
        r"""<p>本接口（CreateSecurityGroupPolicies）用于创建安全组规则（SecurityGroupPolicy）。</p>
    ...
    def DeleteSecurityGroupPolicies(self, request):
        r"""SecurityGroupPolicySet.Version 用于指定要操作的安全组的版本。传入 Version 版本号若不等于当前安全组的最新版本，将返回失败；若不传 Version 则直接删除指定PolicyIndex的规则。
    ...
    def DeleteSubnet(self, request):
        r"""删除子网，若子网为可用区下的默认子网，则默认子网会回退到系统自动创建的默认子网，非用户最新创建的子网。若默认子网不满足需求，可调用设置默认子网接口设置。
    ...
    def DescribeAddressQuota(self, request):
        r"""查询您账户的弹性公网IP（简称 EIP）在当前地域的配额信息
    ...
    def DescribePackingQuotaGroup(self, request):
        r"""使用本接口获取某种机型在某些区域的装箱配额（当使用虚拟机型时，返回的是一组相互关联的装箱配额）。
    ...
    def DescribeRegionIpv6Addresses(self, request):
        r"""该接口（DescribeRegionIpv6Addresses）用于查询ECM地域之下的IPV6地址信息。
    ...
    def DescribeTaskStatus(self, request):
        r"""本接口(DescribeTaskStatus)用于获取异步任务状态
    ...
    def ImportCustomImage(self, request):
        r"""导入自定义镜像，支持 RAW、VHD、QCOW2、VMDK 镜像格式
    ...
    def ModifyDefaultSubnet(self, request):
        r"""修改在一个可用区下创建实例时使用的默认子网（创建实例时，未填写VPC参数时使用的sunbetId）
    ...
    def ModifyIpv6AddressesAttribute(self, request):
        r"""本接口（ModifyIpv6AddressesAttribute）用于修改弹性网卡IPv6地址属性。
    ...
    def ModifyIpv6AddressesBandwidth(self, request):
        r"""该接口(ModifyIpv6AddressesBandwidth)用于修改IPV6地址访问internet的带宽
    ...
    def QueryVpcTaskResult(self, request):
        r"""查询私有网络下Vpc、子网、havip等异步任务请求结果
    ...
    def RebootInstances(self, request):
        r"""只有状态为RUNNING的实例才可以进行此操作；接口调用成功时，实例会进入REBOOTING状态；重启实例成功时，实例会进入RUNNING状态；支持强制重启，强制重启的效果等同于关闭物理计算机的电源开关再重新启动。强制重启可能会导致数据丢失或文件系统损坏，请仅在服务器不能正常重启时使用。
    ...
    def ReleaseIpv6Addresses(self, request):
        r"""本接口（UnassignIpv6Addresses）用于释放弹性网卡IPv6地址。
    ...
    def ReplaceSecurityGroupPolicy(self, request):
        r"""替换单条安全组路由规则, 单个请求中只能替换单个方向的一条规则, 必须要指定索引（PolicyIndex）。
    ...
    def ResetInstances(self, request):
        r"""重装实例，若指定了ImageId参数，则使用指定的镜像重装；否则按照当前实例使用的镜像进行重装；若未指定密码，则密码通过站内信形式随后发送。
    ...
    def ResetInstancesPassword(self, request):
        r"""重置处于运行中状态的实例的密码，需要显式指定强制关机参数ForceStop。如果没有显式指定强制关机参数，则只有处于关机状态的实例才允许执行重置密码操作。
    ...
    def ResetRoutes(self, request):
        r"""对某个路由表名称和所有路由策略（Route）进行重新设置
    ...
    def StartInstances(self, request):
        r"""只有状态为STOPPED的实例才可以进行此操作；接口调用成功时，实例会进入STARTING状态；启动实例成功时，实例会进入RUNNING状态。
    ...
    def UnassignIpv6SubnetCidrBlock(self, request):
        r"""本接口（UnassignIpv6SubnetCidrBlock）用于释放IPv6子网段。
