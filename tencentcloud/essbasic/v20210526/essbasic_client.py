...
class EssbasicClient(AbstractClient):
    ...
    def ChannelCancelUserAutoSignEnableUrl(self, request):
        r"""此接口（ChannelCancelUserAutoSignEnableUrl）用来撤销发送给个人用户的授权签开通链接，撤销后对应的个人用户开通链接失效。若个人用户已经完成开通，将无法撤销。（处方单场景专用，使用此接口请与客户经理确认）
    ...
    def CreateFlowGroupSignReview(self, request):
        r"""1. 在使用[通过多文件创建合同组签署流程](https://qian.tencent.com/developers/partnerApis/startFlows/ChannelCreateFlowGroupByFiles)或[通过多模板创建合同组签署流程](https://qian.tencent.com/developers/partnerApis/startFlows/ChannelCreateFlowGroupByTemplates)创建合同组签署流程时，若指定了参数以下参数为true,则可以调用此接口提交企业内部签署审批结果,即使是授权签署也需要进行审核通过才会进行签署。
    ...
    def CreatePartnerAutoSignAuthUrl(self, request):
        r"""创建一个用于他方【授权签】授权的链接（可选择他方授权或我方授权）。通过这个链接，合作方企业可以直接进入小程序，进行【授权签】授权操作。
    ...
    def CreateSignUrls(self, request):
        r"""本接口用于获取跳转至腾讯电子签小程序的签署链接，签署人点击链接后即可<font color="red">跳转到腾讯电子签小程序</font>完成合同的查看、填写与签署等操作。
    ...
    def ModifyPartnerAutoSignAuthUrl(self, request):
        r"""创建一个用于更新他方授权签授权的链接（可选择他方授权或我方授权）。通过这个链接，合作方企业可以直接进入小程序，进行授权签更新授权（印章）操作。
