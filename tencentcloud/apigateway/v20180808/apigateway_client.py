...
class ApigatewayClient(AbstractClient):
    ...
    def BindEnvironment(self, request):
        r"""本接口（BindEnvironment）用于绑定使用计划到服务或API。
    ...
    def BindIPStrategy(self, request):
        r"""本接口（BindIPStrategy）用于API绑定IP策略。
    ...
    def BindSecretIds(self, request):
        r"""本接口（BindSecretIds）用于为使用计划绑定密钥。
    ...
    def BindSubDomain(self, request):
        r"""本接口（BindSubDomain）用于绑定自定义域名到服务。
    ...
    def CreateApi(self, request):
        r"""本接口（CreateApi）用于创建 API 接口，创建 API 前，用户需要先创建服务，每个 API 都有自己归属的服务。
    ...
    def CreateApiKey(self, request):
        r"""本接口（CreateApiKey）用于创建一对新的 API 密钥。
    ...
    def CreateIPStrategy(self, request):
        r"""本接口（CreateIPStrategy）用于创建服务IP策略。
    ...
    def CreateUsagePlan(self, request):
        r"""本接口（CreateUsagePlan）用于创建使用计划。
    ...
    def DeleteApiApp(self, request):
        r"""本接口（DeleteApiApp）用于删除已经创建的应用。
    ...
    def DeleteApiKey(self, request):
        r"""本接口（DeleteApiKey）用于删除一对 API 密钥。
    ...
    def DeleteIPStrategy(self, request):
        r"""本接口（DeleteIPStrategy）用于删除服务IP策略。
    ...
    def DeleteService(self, request):
        r"""本接口（DeleteService）用于删除 API 网关中某个服务。
    ...
    def DeleteServiceSubDomainMapping(self, request):
        r"""本接口（DeleteServiceSubDomainMapping）用于删除服务中某个环境的自定义域名映射。
    ...
    def DeleteUsagePlan(self, request):
        r"""本接口（DeleteUsagePlan）用于删除使用计划。
    ...
    def DemoteServiceUsagePlan(self, request):
        r"""本接口（DemoteServiceUsagePlan）用于将某个服务在某个环境的使用计划，降级到API上。
        如果服务内没有API不允许进行此操作。
        如果当前环境没有发布，不允许进行此操作。
    ...
    def DescribeAllPluginApis(self, request):
        r"""展示插件相关的API列表，包括已绑定的和未绑定的API信息。
    ...
    def DescribeApi(self, request):
        r"""本接口（DescribeApi）用于查询用户 API 网关的 API 接口的详细信息。​
    ...
    def DescribeApiApp(self, request):
        r"""本接口（DescribeApiApp）用于根据应用ID搜索应用。此接口已下线，如需使用功能请使用DescribeApiAppsStatus接口。
    ...
    def DescribeApiAppsStatus(self, request):
        r"""本接口（DescribeApiAppsStatus）查询应用列表。
    ...
    def DescribeApiBindApiAppsStatus(self, request):
        r"""本接口（DescribeApiBindApiAppsStatus）查询Api绑定的应用列表。
    ...
    def DescribeApiEnvironmentStrategy(self, request):
        r"""本接口（DescribeApiEnvironmentStrategy）用于展示API绑定的限流策略。
    ...
    def DescribeApiForApiApp(self, request):
        r"""本接口（DescribeApiForApiApp）用于应用使用者查询部署于 API 网关的 API 接口的详细信息。​
    ...
    def DescribeApiUsagePlan(self, request):
        r"""本接口（DescribeApiUsagePlan）用于查询服务中 API 使用计划详情。
    ...
    def DescribeApisStatus(self, request):
        r"""本接口（DescribeApisStatus）用于查看一个服务下的某个 API 或所有 API 列表及其相关信息。
    ...
    def DescribeExclusiveInstanceDetail(self, request):
        r"""本接口（DescribeExclusiveInstanceDetail）用于查询独享实例详情信息。
    ...
    def DescribeExclusiveInstances(self, request):
        r"""本接口（DescribeExclusiveInstances）用于查询独享实例列表信息。
    ...
    def DescribeIPStrategy(self, request):
        r"""本接口（DescribeIPStrategy）用于查询IP策略详情。
    ...
    def DescribeIPStrategyApisStatus(self, request):
        r"""本接口（DescribeIPStrategyApisStatus）用于查询IP策略可以绑定的API列表。即服务下所有API和该策略已绑定API的差集。
    ...
    def DescribeIPStrategysStatus(self, request):
        r"""本接口（DescribeIPStrategysStatus）用于查询服务IP策略列表，因为接口名拼写错误，已不推荐使用，请优先使用DescribeIPStrategiesStatus接口。
    ...
    def DescribePlugins(self, request):
        r"""展示插件列表和详情，支持分页，支持按照插件类型查询，支持按照插件ID批量查询，支持按照插件名称查询。
    ...
    def DescribeService(self, request):
        r"""本接口（DescribeService）用于查询一个服务的详细信息、包括服务的描述、域名、协议、创建时间、发布情况等信息。
    ...
    def DescribeServiceEnvironmentList(self, request):
        r"""本接口（DescribeServiceEnvironmentList）用于查询一个服务的环境列表，可查询到此服务下所有环境及其状态。
    ...
    def DescribeServiceEnvironmentReleaseHistory(self, request):
        r"""本接口（DescribeServiceEnvironmentReleaseHistory）用于查询服务环境的发布历史。
    ...
    def DescribeServiceEnvironmentStrategy(self, request):
        r"""本接口（DescribeServiceEnvironmentStrategy）用于展示服务限流策略。
    ...
    def DescribeServiceForApiApp(self, request):
        r"""本接口（DescribeServiceForApiApp）用于应用使用者查询一个服务的详细信息、包括服务的描述、域名、协议等信息。
    ...
    def DescribeServiceReleaseVersion(self, request):
        r"""本接口（DescribeServiceReleaseVersion）查询一个服务下面所有已经发布的版本列表。
    ...
    def DescribeServiceSubDomainMappings(self, request):
        r"""本接口（DescribeServiceSubDomainMappings）用于查询自定义域名的路径映射。
    ...
    def DescribeServiceSubDomains(self, request):
        r"""本接口（DescribeServiceSubDomains）用于查询自定义域名列表。
    ...
    def DescribeServiceUsagePlan(self, request):
        r"""本接口（DescribeServiceUsagePlan）用于查询服务使用计划详情。
    ...
    def DescribeServicesStatus(self, request):
        r"""本接口（DescribeServicesStatus）用于搜索查询某一个服务或多个服务的列表，并返回服务相关的域名、时间等信息。
    ...
    def DescribeUsagePlan(self, request):
        r"""本接口（DescribeUsagePlan）用于查询一个使用计划的详细信息，包括名称、QPS、创建时间绑定的环境等。
    ...
    def DescribeUsagePlanEnvironments(self, request):
        r"""本接口（DescribeUsagePlanEnvironments）用于查询使用计划绑定的环境列表。
    ...
    def DescribeUsagePlanSecretIds(self, request):
        r"""本接口（DescribeUsagePlanSecretIds）用于查询使用计划绑定的密钥列表。
    ...
    def DescribeUsagePlansStatus(self, request):
        r"""本接口（DescribeUsagePlanStatus）用于查询使用计划的列表。
    ...
    def DisableApiKey(self, request):
        r"""本接口（DisableApiKey）用于禁用一对 API 密钥。
    ...
    def EnableApiKey(self, request):
        r"""本接口（EnableApiKey）用于启动一对被禁用的 API 密钥。
    ...
    def ImportOpenApi(self, request):
        r"""本接口（ImportOpenApi）用于将OpenAPI规范定义的API导入到API网关。
    ...
    def ModifyApi(self, request):
        r"""本接口（ModifyApi）用于修改 API 接口，可调用此接口对已经配置的 API 接口进行编辑修改。修改后的 API 需要重新发布 API 所在的服务到对应环境方能生效。
    ...
    def ModifyApiApp(self, request):
        r"""本接口（ModifyApiApp）用于修改已经创建的应用。
    ...
    def ModifyApiEnvironmentStrategy(self, request):
        r"""本接口（ModifyApiEnvironmentStrategy）用于修改API限流策略
    ...
    def ModifyApiIncrement(self, request):
        r"""提供增量更新API能力，主要是给程序调用（区别于ModifyApi，该接口是需要传入API的全量参数，对console使用较友好）
    ...
    def ModifyExclusiveInstance(self, request):
        r"""本接口（ModifyExclusiveInstance）用于修改独享实例信息。​
    ...
    def ModifyIPStrategy(self, request):
        r"""本接口（ModifyIPStrategy）用于修改服务IP策略。
    ...
    def ModifyService(self, request):
        r"""本接口（ModifyService）用于修改服务的相关信息。当服务创建后，服务的名称、描述和服务类型均可被修改。
    ...
    def ModifyServiceEnvironmentStrategy(self, request):
        r"""本接口（ModifyServiceEnvironmentStrategy）用于修改服务限流策略
    ...
    def ModifySubDomain(self, request):
        r"""本接口（ModifySubDomain）用于修改服务的自定义域名设置中的路径映射，可以修改绑定自定义域名之前的路径映射规则。
    ...
    def ModifyUsagePlan(self, request):
        r"""本接口（ModifyUsagePlan）用于修改使用计划的名称，描述及 QPS。
    ...
    def UnBindEnvironment(self, request):
        r"""本接口（UnBindEnvironment）用于将使用计划从特定环境解绑。
    ...
    def UnBindIPStrategy(self, request):
        r"""本接口（UnBindIPStrategy）用于服务解绑IP策略。
    ...
    def UnBindSecretIds(self, request):
        r"""本接口（UnBindSecretIds）用于为使用计划解绑密钥。
    ...
    def UnBindSubDomain(self, request):
        r"""本接口（UnBindSubDomain）用于解绑自定义域名。
    ...
    def UnbindApiApp(self, request):
        r"""本接口（UnbindApiApp）用于解除应用和API绑定。
    ...
    def UpdateApiAppKey(self, request):
        r"""本接口（UpdateApiAppKey）用于更新应用密钥。
    ...
    def UpdateApiKey(self, request):
        r"""本接口（UpdateApiKey）用于更换用户已创建的一对 API 密钥。
    ...
    def UpdateService(self, request):
        r"""本接口（UpdateService）用于从服务已发布的环境中将运行版本切换到特定版本。用户在使用 API 网关创建服务并发布服务到某个环境后，如在开发过程产生多个版本需要切换，此时可调用本接口。
