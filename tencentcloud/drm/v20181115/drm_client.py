...
class DrmClient(AbstractClient):
    ...
    def AddFairPlayPem(self, request):
        r"""本接口用来设置fairplay方案所需的私钥、私钥密钥、ask等信息。
    ...
    def CreateEncryptKeys(self, request):
        r"""该接口用来设置加密的密钥。注意，同一个content id，只能设置一次！
    ...
    def CreateLicense(self, request):
        r"""本接口用来生成DRM方案对应的播放许可证，开发者需提供DRM方案类型、内容类型参数，后台将生成许可证后返回许可证数据
    ...
    def DescribeAllKeys(self, request):
        r"""本接口用来查询指定DRM类型、ContentType的所有加密密钥
    ...
    def DescribeDRMLicense(self, request):
        r"""开发者需要指定使用的DRM类型取值 NORMALAES、和需要加密的Track类型取值 SD,ContentType取值 LiveVideo
    ...
    def DescribeFairPlayPem(self, request):
        r"""该接口用来查询设置的FairPlay私钥校验信息。可用该接口校验设置的私钥与本身的私钥是否一致。
    ...
    def DescribeKeys(self, request):
        r"""开发者需要指定使用的DRM类型、和需要加密的Track类型，后台返回加密使用的密钥
