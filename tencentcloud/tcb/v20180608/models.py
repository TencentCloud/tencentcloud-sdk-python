...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def KeyId(self):
        r"""API Key 的唯一标识符，由系统基于 UUID 自动生成的 Base64 URL 编码字符串。后续对该 API Key 进行删除、修改名称或精确查询操作时，均需使用该值作为定位参数
        :rtype: str
        """
        return self._KeyId
    ...
    @property
    def Name(self):
        r"""API Key 的名称，即创建时传入的 KeyName 参数值。对于 publish_key 类型，该值固定为 publish_key
        :rtype: str
        """
        return self._Name
    ...
    @property
    def ApiKey(self):
        r"""API Key 的令牌值（JWT 格式），用于服务端接口调用时的身份认证。出于安全考虑，仅在创建时返回一次完整明文；后续通过列表查询接口获取时，api_key 类型将进行脱敏处理；publish_key 类型始终返回完整明文。请在创建后妥善保存
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._ApiKey
    ...
    @property
    def ExpireAt(self):
        r"""API Key 的过期时间，格式遵循 ISO 8601 标准。对于 api_key 类型：若创建时未指定有效期（ExpireIn），则该字段不返回，表示永不过期；若指定了有效期，则返回具体的过期时间。对于 publish_key 类型：始终返回，固定为约 2099 年
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
    ...
    @property
    def CreateAt(self):
        r"""API Key 的创建时间，格式遵循 ISO 8601 标准。对于 api_key 类型：为该 Key 实际创建时的时间。对于 publish_key 类型：若环境下已存在 publish_key 记录，则返回首次创建的时间而非本次调用时间
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
...
class BindClsRequest(AbstractModel):
    r"""BindCls请求参数结构体
...
class BindClsResponse(AbstractModel):
    r"""BindCls返回参数结构体
...
class CreateApiKeyRequest(AbstractModel):
    ...
    @property
    def EnvId(self):
        r"""环境 ID，用于标识该密钥归属的云开发环境，不同环境之间的数据相互隔离
        :rtype: str
        """
        return self._EnvId
    ...
    @property
    def KeyType(self):
        r"""密钥类型。可选值：api_key（服务端调用使用的 API 密钥，具有完整权限，请勿暴露在客户端）、publish_key（客户端使用的公开密钥，权限受限，可安全用于前端或移动端）。
        :rtype: str
        """
    ...
    @property
    def KeyName(self):
        r"""密钥的自定义名称，用于在管理列表中标识和区分不同的密钥，建议填写能体现用途或归属的描述性名称，例如：server-prod、mobile-test
        :rtype: str
        """
    ...
    @property
    def ExpireIn(self):
        r"""密钥的有效期，单位为秒，最短不得低于 7200 秒。超过有效期后密钥将自动失效。不设置或设置为 0 则表示永不过期，建议根据安全需求合理设置有效期
        :rtype: int
        """
        return self._ExpireIn

    @ExpireIn.setter
    def ExpireIn(self, ExpireIn):
    ...
    def _deserialize(self, params):
        ...
        self._ExpireIn = params.get("ExpireIn")
        memeber_set = set(params.keys())
        for name, value in vars(self).items():
            property_name = name[1:]
            if property_name in memeber_set:
...
class CreateApiKeyResponse(AbstractModel):
    ...
    def __init__(self):
        r"""
        :param _KeyId: API Key 的唯一标识符，由系统基于 JWT Access Token Hash 自动生成。后续对该 API Key 进行查询、修改名称或删除操作时，均需使用该值作为定位参数
        :type KeyId: str
        :param _Name: API Key 的名称，即创建时传入的 KeyName 参数值。对于 publish_key 类型，该值固定为 publish_key
        :type Name: str
        :param _ApiKey: API Key 的令牌值（JWT 格式），用于服务端接口调用时的身份认证。出于安全考虑，仅在创建时返回一次完整明文；后续通过列表查询接口获取时，api_key 类型将进行脱敏处理；publish_key 类型始终返回完整明文。请在创建后妥善保存
注意：此字段可能返回 null，表示取不到有效值。
        :type ApiKey: str
        :param _ExpireAt: API Key 的过期时间。对于 api_key 类型：若创建时未指定有效期，则该字段不返回，表示永不过期；若指定了有效期，则返回具体的过期时间。对于 publish_key 类型：始终返回，固定为 2099 年
注意：此字段可能返回 null，表示取不到有效值。
        :type ExpireAt: str
        :param _CreateAt: API Key 的创建时间。对于 api_key 类型：为实际创建该 Key 时的时间。对于 publish_key 类型：若环境下已存在 publish_key，则返回首次创建的时间而非本次调用时间
注意：此字段可能返回 null，表示取不到有效值。
        :type CreateAt: str
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
    ...
    @property
    def KeyId(self):
        r"""API Key 的唯一标识符，由系统基于 JWT Access Token Hash 自动生成。后续对该 API Key 进行查询、修改名称或删除操作时，均需使用该值作为定位参数
        :rtype: str
        """
        return self._KeyId
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def ExpireAt(self):
        r"""API Key 的过期时间。对于 api_key 类型：若创建时未指定有效期，则该字段不返回，表示永不过期；若指定了有效期，则返回具体的过期时间。对于 publish_key 类型：始终返回，固定为 2099 年
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._ExpireAt
    ...
    @property
    def CreateAt(self):
        r"""API Key 的创建时间。对于 api_key 类型：为实际创建该 Key 时的时间。对于 publish_key 类型：若环境下已存在 publish_key，则返回首次创建的时间而非本次调用时间
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._CreateAt
...
class CreateHTTPServiceRouteRequest(AbstractModel):
    r"""CreateHTTPServiceRoute请求参数结构体
...
class CreateHTTPServiceRouteResponse(AbstractModel):
    r"""CreateHTTPServiceRoute返回参数结构体
...
class CreatePlatformEnvRequest(AbstractModel):
    r"""CreatePlatformEnv请求参数结构体
...
class CreatePlatformEnvResponse(AbstractModel):
    r"""CreatePlatformEnv返回参数结构体
...
class DailyUsageList(AbstractModel):
    r"""每日用量详情结构
    ...
    @property
    def Date(self):
        r"""<p>资源点用量日期</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._Date
...
class DeleteApiKeyRequest(AbstractModel):
    ...
    @property
    def KeyId(self):
        r"""密钥的唯一标识符，用于精确定位指定的 API 密钥。可通过查询密钥列表接口获取
        :rtype: str
        """
        return self._KeyId
...
class DeleteHTTPServiceRouteResponse(AbstractModel):
    r"""DeleteHTTPServiceRoute返回参数结构体
...
class DescribeApiKeyListRequest(AbstractModel):
    ...
    @property
    def PageNumber(self):
        r"""分页查询的页码，从 1 开始。与 PageSize 配合使用，不传则默认返回第 1 页
        :rtype: int
        """
    ...
    @property
    def PageSize(self):
        r"""分页查询每页返回的记录条数。与 PageNumber 配合使用，不传则使用系统默认值
        :rtype: int
        """
    ...
    @property
    def KeyType(self):
        r"""密钥类型过滤条件。可选值：api_key（服务端调用使用的 API 密钥，具有完整权限）、publish_key（客户端使用的公开密钥，权限受限）密钥类型过滤条件。不传默认值为api_key
        :rtype: str
        """
        return self._KeyType
...
class DescribeApiKeyListResponse(AbstractModel):
    ...
    def __init__(self):
        r"""
        :param _Data: API Key列表
        :type Data: list of ApiKeyToken
        :param _Total: 总数
        :type Total: int
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
...
class DescribeGatewayVersionsRequest(AbstractModel):
    r"""DescribeGatewayVersions请求参数结构体
...
class DescribeGatewayVersionsResponse(AbstractModel):
    r"""DescribeGatewayVersions返回参数结构体
...
class DescribeHTTPServiceRouteRequest(AbstractModel):
    ...
    @property
    def Filters(self):
        r"""过滤条件。Key的含义参考对应字段，Value精确匹配。可过滤: Domain、Path、DomainType、UpstreamResourceType。可过滤的Values单条不超过100
...
class DescribeHTTPServiceRouteResponse(AbstractModel):
    r"""DescribeHTTPServiceRoute返回参数结构体

    """

    def __init__(self):
        r"""
        :param _Domains: 域名路由信息列表
        :type Domains: list of HTTPServiceDomain
        :param _OriginDomain: 自定义接入的源站域名（HTTPService接入层域名）
        :type OriginDomain: str
        :param _TotalCount: 域名总数，分页查询使用总数判断是否已经拉取到所有数据
        :type TotalCount: int
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
...
class DescribePlatformAccountCircleRequest(AbstractModel):
    r"""DescribePlatformAccountCircle请求参数结构体

    """


class DescribePlatformAccountCircleResponse(AbstractModel):
    r"""DescribePlatformAccountCircle返回参数结构体

    """

    def __init__(self):
        r"""
        :param _StartTime: <p>套餐计费周期开始时间</p>
        :type StartTime: str
        :param _EndTime: <p>套餐计费周期结束时间</p>
        :type EndTime: str
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
...
class DescribePlatformCreditsUsageDetailRequest(AbstractModel):
    r"""DescribePlatformCreditsUsageDetail请求参数结构体
    ...
    @property
    def StartDate(self):
        r"""<p>开始日期</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._StartDate
    ...
    @property
    def EndDate(self):
        r"""<p>结束日期</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._EndDate
...
class DescribePlatformCreditsUsageDetailResponse(AbstractModel):
    r"""DescribePlatformCreditsUsageDetail返回参数结构体
    ...
    def __init__(self):
        r"""
        :param _Usages: <p>用量数据</p>
        :type Usages: list of PlatformPkgCreditsUsage
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
        self._Usages = None
    ...
    def Usages(self):
        r"""<p>用量数据</p>
        :rtype: list of PlatformPkgCreditsUsage
        """
        return self._Usages
    ...
    def _deserialize(self, params):
        if params.get("Usages") is not None:
            self._Usages = []
            for item in params.get("Usages"):
                obj = PlatformPkgCreditsUsage()
                obj._deserialize(item)
                self._Usages.append(obj)
        self._RequestId = params.get("RequestId")


class DescribePlatformCreditsUsageRequest(AbstractModel):
    r"""DescribePlatformCreditsUsage请求参数结构体
    ...
    @property
    def StartDate(self):
        r"""<p>开始日期</p><p>参数格式：2025-09-22</p>
        :rtype: str
        """
        return self._StartDate
    ...
    @property
    def EndDate(self):
        r"""<p>结束日期</p><p>参数格式：2025-09-22</p>
        :rtype: str
        """
        return self._EndDate
...
class DescribePlatformCreditsUsageResponse(AbstractModel):
    r"""DescribePlatformCreditsUsage返回参数结构体

    """

    def __init__(self):
        r"""
        :param _DeductValueCount: <p>资源点套餐内用量总和</p>
        :type DeductValueCount: float
        :param _PackageDeductValueCount: <p>资源点资源包用量总和</p>
        :type PackageDeductValueCount: float
        :param _ReportValueCount: <p>资源点按量用量总和</p>
        :type ReportValueCount: float
        :param _DailyList: <p>每日消耗具体数据</p>
        :type DailyList: list of PlatformCreditsUsageDaily
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
        self._DeductValueCount = None
        self._PackageDeductValueCount = None
        self._ReportValueCount = None
        self._DailyList = None
        self._RequestId = None

    @property
    def DeductValueCount(self):
        r"""<p>资源点套餐内用量总和</p>
        :rtype: float
        """
        return self._DeductValueCount

    @DeductValueCount.setter
    def DeductValueCount(self, DeductValueCount):
        self._DeductValueCount = DeductValueCount

    @property
    def PackageDeductValueCount(self):
        r"""<p>资源点资源包用量总和</p>
        :rtype: float
        """
        return self._PackageDeductValueCount

    @PackageDeductValueCount.setter
    def PackageDeductValueCount(self, PackageDeductValueCount):
        self._PackageDeductValueCount = PackageDeductValueCount

    @property
    def ReportValueCount(self):
        r"""<p>资源点按量用量总和</p>
        :rtype: float
        """
        return self._ReportValueCount

    @ReportValueCount.setter
    def ReportValueCount(self, ReportValueCount):
        self._ReportValueCount = ReportValueCount

    @property
    def DailyList(self):
        r"""<p>每日消耗具体数据</p>
        :rtype: list of PlatformCreditsUsageDaily
        """
        return self._DailyList

    @DailyList.setter
    def DailyList(self, DailyList):
    ...
    def _deserialize(self, params):
        self._DeductValueCount = params.get("DeductValueCount")
        self._PackageDeductValueCount = params.get("PackageDeductValueCount")
        self._ReportValueCount = params.get("ReportValueCount")
        if params.get("DailyList") is not None:
            self._DailyList = []
            for item in params.get("DailyList"):
                obj = PlatformCreditsUsageDaily()
                obj._deserialize(item)
                self._DailyList.append(obj)
        self._RequestId = params.get("RequestId")


class DescribePlatformEnvUsageRequest(AbstractModel):
    r"""DescribePlatformEnvUsage请求参数结构体
    ...
    @property
    def StartDate(self):
        r"""<p>查询用量起始时间</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._StartDate
    ...
    @property
    def EndDate(self):
        r"""<p>查询用量结束时间</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._EndDate
    ...
    @property
    def ResourceTypes(self):
        r"""<p>资源类型</p><p>枚举值：</p><ul><li>Storage： 云存储</li><li>Function： 云函数</li><li>Database： 数据库</li></ul>
        :rtype: list of str
        """
        return self._ResourceTypes
...
class DescribePlatformEnvUsageResponse(AbstractModel):
    r"""DescribePlatformEnvUsage返回参数结构体

    """

    def __init__(self):
        r"""
        :param _Resources: <p>资源用量信息</p>
        :type Resources: list of PlatformResUsageItem
        :param _TotalCredits: <p>资源点</p>
        :type TotalCredits: int
        :param _CreditsScale: <p>资源点取整倍数</p>
        :type CreditsScale: int
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
        self._Resources = None
        self._TotalCredits = None
        self._CreditsScale = None
        self._RequestId = None

    @property
    def Resources(self):
        r"""<p>资源用量信息</p>
        :rtype: list of PlatformResUsageItem
        """
        return self._Resources
    ...
    @TotalCredits.setter
    def TotalCredits(self, TotalCredits):
        self._TotalCredits = TotalCredits

    @property
    def CreditsScale(self):
        r"""<p>资源点取整倍数</p>
        :rtype: int
        """
        return self._CreditsScale

    @CreditsScale.setter
    def CreditsScale(self, CreditsScale):
        self._CreditsScale = CreditsScale
...
class DescribePlatformsRequest(AbstractModel):
    ...
    def __init__(self):
        ...
        :type PlatformIds: list of str
        :param _Limit: <p>分页限制</p><p>取值范围：[10, 100]</p><p>默认值：10</p>
        :type Limit: int
        :param _Offset: <p>分页偏移量</p><p>默认值：0</p>
        :type Offset: int
        """
        self._PlatformIds = None
        self._Limit = None
        self._Offset = None

    @property
    def PlatformIds(self):
        r"""<p>平台版套餐id列表</p><p>默认值：若不指定，则分页返回当前账号下所有平台版资源</p>
        :rtype: list of str
        """
        return self._PlatformIds

    @PlatformIds.setter
    def PlatformIds(self, PlatformIds):
        self._PlatformIds = PlatformIds

    @property
    def Limit(self):
        r"""<p>分页限制</p><p>取值范围：[10, 100]</p><p>默认值：10</p>
        :rtype: int
        """
        return self._Limit
    ...
    def _deserialize(self, params):
        self._PlatformIds = params.get("PlatformIds")
        self._Limit = params.get("Limit")
        self._Offset = params.get("Offset")
        memeber_set = set(params.keys())
        for name, value in vars(self).items():
...
class DescribePlatformsResponse(AbstractModel):
    ...
    def __init__(self):
        r"""
        :param _PlatformList: <p>平台版资源列表</p>
        :type PlatformList: list of PlatformInfo
        :param _Total: <p>总数</p>
        :type Total: int
        :param _RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        :type RequestId: str
        """
        self._PlatformList = None
        self._Total = None
        self._RequestId = None
    ...
    @PlatformList.setter
    def PlatformList(self, PlatformList):
        self._PlatformList = PlatformList
    ...
    def _deserialize(self, params):
        if params.get("PlatformList") is not None:
            self._PlatformList = []
            for item in params.get("PlatformList"):
                obj = PlatformInfo()
                obj._deserialize(item)
                self._PlatformList.append(obj)
        self._Total = params.get("Total")
        self._RequestId = params.get("RequestId")
...
class DestroyPlatformEnvRequest(AbstractModel):
    r"""DestroyPlatformEnv请求参数结构体
...
class DestroyPlatformEnvResponse(AbstractModel):
    r"""DestroyPlatformEnv返回参数结构体
...
class HTTPServiceCacheKeyParams(AbstractModel):
    ...
    def QueryStringValues(self):
        r"""<p>参数名列表</p><p>入参限制：最多 30 项，单项 1~128 字节</p>
        :rtype: list of str
        """
...
class HTTPServiceDomain(AbstractModel):
    ...
    @property
    def AccessType(self):
        r"""<p>绑定类型。默认DIRECT。DIRECT: 直连到HTTP访问服务， CDN: 接入云开发CDN，CUSTOM: 自定义接入类型（其他CDN或者WAF）</p><p>枚举值：</p><ul><li>DIRECT： 直连到HTTP访问服务</li><li>CDN： 接入云开发CDN（即将下线）</li><li>EO： 接入云开发EdgeOne</li><li>CUSTOM： 自定义接入类型（其他CDN或者WAF）</li><li>NONE： 不接入，当使用泛域名接入时，可通过NONE来接入子域名，子域名证书及协议继承泛域名</li></ul><p>默认值：DIRECT</p>
        :rtype: str
        """
...
class HTTPServiceDomainParam(AbstractModel):
    r"""创建或修改HTTP访问服务输入的域名信息，修改HTTP访问服务域名时对应字段不传参数表示不需要修改。
    ...
    @property
    def AccessType(self):
        r"""<p>绑定类型</p><p>枚举值：</p><ul><li>DIRECT： 直连到HTTP访问服务</li><li>CDN： 接入云开发CDN（即将下线）</li><li>EO： 接入云开发EdgeOne</li><li>NONE： 不接入，当使用泛域名接入时，可通过NONE来接入子域名，子域名证书及协议继承泛域名</li></ul><p>默认值：DIRECT</p>
        :rtype: str
        """
        return self._AccessType
...
class HTTPServiceExtension(AbstractModel):
    ...
    @property
    def Cache(self):
        r"""<p>HTTPService 缓存配置，仅限自定义域名配置。包含Cache 节点缓存 / MaxAge 浏览器缓存 / CacheKey 自定义缓存键</p>
...
class HTTPServicePathRewrite(AbstractModel):
    r"""HTTP访问服务路径重写配置
    ...
    @property
    def Prefix(self):
        r"""路径前缀重写。StaticStorePrefix、Prefix只能填一个
        :rtype: str
        """
        return self._Prefix
...
class HTTPServiceQPSPerClient(AbstractModel):
    r"""http访问服务客户端限频
...
class HTTPServiceRoute(AbstractModel):
    r"""查询HTTP访问服务输出路由信息
    ...
    def __init__(self):
        ...
        self._UpstreamResourceName = None
        self._EnableSafeDomain = None
    ...
    @UpstreamResourceType.setter
    def UpstreamResourceType(self, UpstreamResourceType):
        self._UpstreamResourceType = UpstreamResourceType
    ...
    @UpstreamResourceName.setter
    def UpstreamResourceName(self, UpstreamResourceName):
        self._UpstreamResourceName = UpstreamResourceName
    ...
    @EnableSafeDomain.setter
    def EnableSafeDomain(self, EnableSafeDomain):
        self._EnableSafeDomain = EnableSafeDomain
    ...
    @EnablePathTransmission.setter
    def EnablePathTransmission(self, EnablePathTransmission):
        self._EnablePathTransmission = EnablePathTransmission
    ...
    def _deserialize(self, params):
        ...
        if params.get("PathRewrite") is not None:
            self._PathRewrite = HTTPServicePathRewrite()
            self._PathRewrite._deserialize(params.get("PathRewrite"))
        self._UpstreamResourceType = params.get("UpstreamResourceType")
        self._UpstreamResourceName = params.get("UpstreamResourceName")
        self._EnableSafeDomain = params.get("EnableSafeDomain")
        self._EnableAuth = params.get("EnableAuth")
        self._EnablePathTransmission = params.get("EnablePathTransmission")
...
class HTTPServiceRouteParam(AbstractModel):
    r"""创建或修改HTTP访问服务输入的路由信息，修改HTTP访问服务路由信息时对应字段不传参数表示不用修改。
...
class HTTPServiceRouteQPSPolicy(AbstractModel):
    r"""云开发路由限频策略
    ...
    def QPSPerClient(self):
        r"""客户端限频配置
        :rtype: :class:`tencentcloud.tcb.v20180608.models.HTTPServiceQPSPerClient`
        """
...
class HTTPServiceRuleCondition(AbstractModel):
    ...
    @property
    def Values(self):
        r"""<p>Values 匹配值集合，Values 内任一命中即认为条件成立（OR 语义）</p><p>入参限制：单项 1~1024 字节，最多 30 条</p>
        :rtype: list of str
        """
        return self._Values
...
class ModifyHTTPServiceRouteRequest(AbstractModel):
    r"""ModifyHTTPServiceRoute请求参数结构体
...
class ModifyHTTPServiceRouteResponse(AbstractModel):
    r"""ModifyHTTPServiceRoute返回参数结构体
...
class ModifyPlatformEnvRequest(AbstractModel):
    r"""ModifyPlatformEnv请求参数结构体
    ...
    @property
    def Status(self):
        r"""<p>环境状态</p><p>枚举值：</p><ul><li>ENABLE： 启用环境</li><li>DISABLE： 禁用环境</li></ul>
        :rtype: str
        """
        return self._Status
...
class ModifyPlatformEnvResponse(AbstractModel):
    r"""ModifyPlatformEnv返回参数结构体
...
class PlatFormResourceInfo(AbstractModel):
    r"""平台版资源信息
    ...
    @property
    def ResType(self):
        r"""<p>资源类系</p><p>枚举值：</p><ul><li>log： 日志</li><li>storage： 云存储</li><li>hosting： 静态托管</li></ul>
        :rtype: str
        """
    ...
    @property
    def Status(self):
        r"""<p>资源状态</p><p>枚举值：</p><ul><li>0： 正常</li><li>5： 初始化中</li></ul>
        :rtype: int
        """
        return self._Status
    ...
    def _deserialize(self, params):
        ...
        self._ResName = params.get("ResName")
...
class PlatformCreditsUsageDaily(AbstractModel):
    r"""平台版本消耗数据
    ...
    def __init__(self):
        ...
        self._PackageDeductValue = None
        self._ReportValue = None
        self._OriginCredits = None

    @property
    def Date(self):
        r"""<p>数据日期</p><p>参数格式：YYYY-MM-DD</p>
        :rtype: str
        """
        return self._Date
    ...
    @property
    def OriginCredits(self):
        r"""<p>资源点原价消耗</p>
        :rtype: float
        """
        return self._OriginCredits

    @OriginCredits.setter
    def OriginCredits(self, OriginCredits):
        self._OriginCredits = OriginCredits
    ...
    def _deserialize(self, params):
        ...
        self._PackageDeductValue = params.get("PackageDeductValue")
        self._ReportValue = params.get("ReportValue")
        self._OriginCredits = params.get("OriginCredits")
        memeber_set = set(params.keys())
        for name, value in vars(self).items():
            property_name = name[1:]
            if property_name in memeber_set:
...
class PlatformInfo(AbstractModel):
    ...
    def __init__(self):
        ...
        :type Resources: list of PlatFormResourceInfo
    ...
    @property
    def BillStatus(self):
        r"""<p>计费状态</p><p>枚举值：</p><ul><li>normal： 正常</li><li>isolated： 已隔离</li><li>destroyed： 已销毁</li></ul>
        :rtype: str
        """
        return self._BillStatus

    @BillStatus.setter
    def BillStatus(self, BillStatus):
        self._BillStatus = BillStatus

    @property
    def Status(self):
        r"""<p>套餐资源状态</p><p>枚举值：</p><ul><li>0： 可用</li><li>5： 发货中</li></ul>
        :rtype: int
        """
        return self._Status
    ...
    @property
    def BillTime(self):
        r"""<p>购买时间</p><p>参数格式：YYYY-MM-DD hh:mm:ss</p>
        :rtype: str
        """
        return self._BillTime

    @BillTime.setter
    def BillTime(self, BillTime):
        self._BillTime = BillTime

    @property
    def ExpireTime(self):
        r"""<p>套餐过期时间</p><p>参数格式：YYYY-MM-DD hh:mm:ss</p>
        :rtype: str
        """
        return self._ExpireTime
    ...
    @property
    def IsAutoRenew(self):
        r"""<p>是否自动续费</p><p>枚举值：</p><ul><li>0： 未设置</li><li>1： 自动续费</li><li>2： 设置为到期不续费</li></ul>
        :rtype: int
        """
        return self._IsAutoRenew
    ...
    def Resources(self):
        r"""<p>资源信息列表</p>
        :rtype: list of PlatFormResourceInfo
        """
        return self._Resources
    ...
    @property
    def Region(self):
        r"""<p>所属地域</p><p>枚举值：</p><ul><li>ap-shanghai： 上海</li><li>ap-singapore： 新加坡</li></ul>
        :rtype: str
        """
        return self._Region
    ...
    def _deserialize(self, params):
        ...
        self._Alias = params.get("Alias")
        self._PackageId = params.get("PackageId")
        self._BillStatus = params.get("BillStatus")
        self._Status = params.get("Status")
        self._Spec = params.get("Spec")
        self._BillTime = params.get("BillTime")
        self._ExpireTime = params.get("ExpireTime")
        self._IsAutoRenew = params.get("IsAutoRenew")
        if params.get("Resources") is not None:
            self._Resources = []
            for item in params.get("Resources"):
                obj = PlatFormResourceInfo()
                obj._deserialize(item)
                self._Resources.append(obj)
        self._Region = params.get("Region")
        memeber_set = set(params.keys())
...
class PlatformMetricUsageItem(AbstractModel):
    r"""平台版指标用量信息
    ...
    def __init__(self):
        ...
        :type OriginalResourceType: str
        :param _OriginalMetricName: <p>原始指标</p>
        :type OriginalMetricName: str
        ...
        :type DailyUsageList: list of DailyUsageList
        """
        self._MetricName = None
        self._OriginalResourceType = None
        self._OriginalMetricName = None
        self._UsageValue = None
        self._UsageUnit = None
        self._Credits = None
        self._DailyUsageList = None
    ...
    @property
    def OriginalResourceType(self):
        r"""<p>原始资源类型</p><p>枚举值：</p><ul><li>COS： 对象存储</li></ul>
        :rtype: str
        """
        return self._OriginalResourceType

    @OriginalResourceType.setter
    def OriginalResourceType(self, OriginalResourceType):
        self._OriginalResourceType = OriginalResourceType

    @property
    def OriginalMetricName(self):
        r"""<p>原始指标</p>
        :rtype: str
        """
        return self._OriginalMetricName

    @OriginalMetricName.setter
    def OriginalMetricName(self, OriginalMetricName):
        self._OriginalMetricName = OriginalMetricName
    ...
    @property
    def DailyUsageList(self):
        r"""<p>用量按日明细列表</p>
        :rtype: list of DailyUsageList
        """
        return self._DailyUsageList

    @DailyUsageList.setter
    def DailyUsageList(self, DailyUsageList):
        self._DailyUsageList = DailyUsageList


    def _deserialize(self, params):
        self._MetricName = params.get("MetricName")
        self._OriginalResourceType = params.get("OriginalResourceType")
        self._OriginalMetricName = params.get("OriginalMetricName")
        self._UsageValue = params.get("UsageValue")
        self._UsageUnit = params.get("UsageUnit")
        self._Credits = params.get("Credits")
        if params.get("DailyUsageList") is not None:
            self._DailyUsageList = []
            for item in params.get("DailyUsageList"):
                obj = DailyUsageList()
                obj._deserialize(item)
                self._DailyUsageList.append(obj)
        memeber_set = set(params.keys())
        for name, value in vars(self).items():
            property_name = name[1:]
            if property_name in memeber_set:
...
class PlatformPkgCreditsUsage(AbstractModel):
    r"""模块内平台版资源点用量及原始用量数据结构
...
class PlatformResUsageItem(AbstractModel):
    r"""平台版资源用量信息
    ...
    def __init__(self):
        ...
        :type Metrics: list of PlatformMetricUsageItem
        """
        self._ResourceType = None
        self._TotalCredits = None
        self._Metrics = None
    ...
    @property
    def ResourceType(self):
        r"""<p>资源类型</p><p>枚举值：</p><ul><li>Storage： 云存储</li><li>Function： 云函数</li></ul>
        :rtype: str
        """
        return self._ResourceType
    ...
    def Metrics(self):
        r"""<p>指标用量信息</p>
        :rtype: list of PlatformMetricUsageItem
        """
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def Vendor(self):
        r"""<p>短信服务商类型</p><p>枚举值：</p><ul><li>TENCENT_CN： 腾讯云国内短信</li><li>TENCENT_INTL： 腾讯云国际短信</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._Vendor
    ...
    @property
    def TemplateId(self):
        r"""<p>短信服务商侧申请并审核通过的模板ID。</p><ul><li>腾讯云短信模板ID可前往 <a href="https://console.cloud.tencent.com/smsv2/csms-template">腾讯云国内短信</a> 或 <a href="https://console.cloud.tencent.com/smsv2/isms-template">国际/港澳台短信</a> 的正文模板管理查看，若向境外手机号发送短信，仅支持使用国际/港澳台短信模板。</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
    ...
    @property
    def SdkAppId(self):
        r"""<p>短信服务商侧为应用分配的ID，按照服务商文档和要求需要此参数的，填写该参数。</p><ul><li>腾讯云国内短信和国际短信此参数必填，可以在<a href="https://console.cloud.tencent.com/smsv2/app-manage">短信控制台</a>的应用列表中查看对应的应用id。</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
    ...
    @property
    def SignName(self):
        r"""<p>短信服务商侧申请并审核通过的签名，按照服务商的文档和要求填写。</p><ul><li>腾讯云短信服务商，签名信息可前往 <a href="https://console.cloud.tencent.com/smsv2/csms-sign">国内短信</a> 或 <a href="https://console.cloud.tencent.com/smsv2/isms-sign">国际/港澳台短信</a> 的签名管理查看。<br> 注意：<ol><li>发送国内短信该参数必填，且需填写签名内容而非签名ID。</li><li>发送国际/港澳台短信该参数非必填。</li></ol></li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def SecretKey(self):
        r"""<p>调用短信服务商发送短信接口的调用api秘钥对应的秘钥Key。</p><ul><li>腾讯云的调用api秘钥在腾讯云控制台获取，建议使用子账号的秘钥ID, 并且按照最小权限配置。平台对于调用api秘钥key是加密存储的，不会明文存储。</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._SecretKey
    ...
    @property
    def SenderId(self):
        r"""<p>短信服务商侧自定义短信发送的senderID，按照服务商文档和要求传参。</p><ul><li>仅国际化场景使用。部分国家/运营商支持自定义 Sender ID替代默认主叫号码。国内站点忽略此字段。</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
    ...
    @property
    def TemplateExtendParam(self):
        r"""<p>当短信自定义模板含多个占位符时，平台只负责生成验证码值，其余占位符由调用方在此提供。</p><ul><li>无需提供验证码对应的占位的值，验证码由云开发平台侧生成。</li><li>如果是命名占位的服务商的短信模板，这里的参数按照需要对应的占位的key和value，会按照对应的key和value在发送短信时，填充到模板中。</li><li>如果是序号占位的服务商的短信模板，这里的参数不需要key, 只需要填写对应的value, 会按照填写的顺序依次填充到模板中。</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: list of SMSTemplateParams
        """
        return self._TemplateExtendParam

    @TemplateExtendParam.setter
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    def _deserialize(self, params):
        ...
        if params.get("TemplateExtendParam") is not None:
            self._TemplateExtendParam = []
            for item in params.get("TemplateExtendParam"):
...
class SMSTemplateParams(AbstractModel):
    r"""自定义短信模板中的自定义参数
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def Key(self):
        r"""<p>短信模板的自定义参数的key。如果短信厂商的自定义参数按照命名占位的，才需要此参数；如果按照序号占位的， 不需要此参数。</p><p>腾讯云短信是按照序号占位的，不需要此参数。</p>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._Key
...
class VerificationConfig(AbstractModel):
    r"""登录短信验证码发送配置。用于管理登录时使用的短信验证码发送的通道相关设置，目前提供云开发默认短信包和客户自定义短信包，自定义短信包可以通过自定义apis或者自定义短信模板的方式接入，推荐使用云开发默认短信包，方便快捷。
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def Type(self):
        r"""<p>短信验证码发送通道类型。</p><p>枚举值：</p><ul><li>default： 使用默认云开发短信包发送短信</li><li>apis： 使用云开发自定义 APIs 作为短信发送通道，需配合 Name 和 Method 参数使用。不传则不修改当前配置。</li><li>template： 自定义短信模板配置，需要配置TemplateProvider</li><li>function： 使用云函数作为短信发送通道，需配合 CloudFunction 参数使用</li></ul>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._Type
    ...
    @property
    def Name(self):
        r"""<p>自定义 APIs 数据源唯一标识，当 Type 为 apis 时必填。用于定位微搭 APIs 中对应的数据源。</p>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._Name
    ...
    @property
    def Method(self):
        r"""<p>自定义 APIs 方法名，当 Type 为 apis 时必填。指定微搭 APIs 中用于发送验证码的方法。</p>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: str
        """
        return self._Method
    ...
    def SmsDayLimit(self):
        r"""<p>单个手机号每日短信发送上限。默认值为 30，传 -1 表示不限制，如果设置为不限制，需要注意恶意攻击，导致短信套餐用量计费问题。仅支持正整数或 -1。不传则不修改当前配置。</p>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: int
        """
    ...
    def TemplateProvider(self):
        r"""<p>自定义短信服务商模板配置</p>
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: :class:`tencentcloud.tcb.v20180608.models.SMSProviderTemplateConfig`
        """
        return self._TemplateProvider

    @TemplateProvider.setter
...
class WxGatewayCustomConfig(AbstractModel):
    ...
    def SourceIpType(self):
        r"""获取源ip方式，PPV1(Proxy Protocol V1)、PPV2(Proxy Protocol V2)、TOA(tcp option address)
        :rtype: str
        """
