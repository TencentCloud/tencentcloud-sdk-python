...
class NlpClient(AbstractClient):
    ...
    def TestingTextGeneration(self, request):
        ...
        :type request: :class:`tencentcloud.nlp.v20190408.models.TestingTextGenerationRequest`
        :rtype: :class:`tencentcloud.nlp.v20190408.models.TestingTextGenerationResponse`

        """
        try:
            params = request._serialize()
            headers = request.headers
            body = self.call("TestingTextGeneration", params, headers=headers)
            response = json.loads(body)
            model = models.TestingTextGenerationResponse()
            model._deserialize(response["Response"])
            return model
        except Exception as e:
            if isinstance(e, TencentCloudSDKException):
