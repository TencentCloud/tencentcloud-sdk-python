...
class AnalyzeDenseLandmarksRequest(AbstractModel):
    r"""AnalyzeDenseLandmarks请求参数结构体
...
- 支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    def FaceModelVersion(self):
        r"""人脸识别服务所用的算法模型版本。本接口仅支持 “3.0“ 输入。
        :rtype: str
        """
...
class AnalyzeDenseLandmarksResponse(AbstractModel):
    r"""AnalyzeDenseLandmarks返回参数结构体
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def FaceId(self):
        r"""人脸ID，仅在SearchFaces/SearchFacesReturnsByGroup接口返回时有效。人员搜索类接口采用融合特征方式检索，该字段无意义
        :rtype: str
        """
        return self._FaceId
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def ImageA(self):
        r"""A 图片 base64 数据，base64 编码后大小不可超过5M。
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
        :rtype: str
        """
    ...
    @property
    def ImageB(self):
        r"""B 图片 base64 数据，base64 编码后大小不可超过5M。
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
        :rtype: str
        """
    ...
    @property
    def UrlA(self):
        r"""A 图片的 Url ，对应图片 base64 编码后大小不可超过5M。
A 图片的 Url、Image必须提供一个，如果都提供，只使用 Url。 
图片存储于腾讯云的Url可保障更高下载速度和稳定性，建议图片存储于腾讯云。 
非腾讯云存储的Url速度和稳定性可能受一定影响。
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    @property
    def UrlB(self):
        r"""B 图片的 Url ，对应图片 base64 编码后大小不可超过5M。
B 图片的 Url、Image必须提供一个，如果都提供，只使用 Url。 
图片存储于腾讯云的Url可保障更高下载速度和稳定性，建议图片存储于腾讯云。 
非腾讯云存储的Url速度和稳定性可能受一定影响。
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def NeedRotateDetection(self):
        r"""是否开启图片旋转识别支持。0为不开启，1为开启。默认为0。本参数的作用为，当图片中的人脸被旋转且图片没有exif信息时，如果不开启图片旋转识别支持则无法正确检测、识别图片中的人脸。若您确认图片包含exif信息或者您确认输入图中人脸不会出现被旋转情况，请不要开启本参数。开启后，整体耗时将可能增加数百毫秒。
        :rtype: int
        """
        return self._NeedRotateDetection
...
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
    ...
    def FaceMatchThreshold(self):
        r"""只有和该人员已有的人脸相似度超过FaceMatchThreshold值的人脸，才能增加人脸成功。 
默认值60分。取值范围[0,100] 。
        :rtype: float
        """
...
例， Urls 中 有 3 个 url，第二个 url 失败，则 SucIndexes 值为 [0,2] 。
    ...
    @property
    def RetCode(self):
        r"""每张人脸图片添加结果，-1101 代表未检测到人脸，-1102 代表图片解码失败，-1109 代表图片尺寸过大或者过小， 
-1601代表不符合图片质量控制要求, -1604 代表人脸相似度没有超过FaceMatchThreshold。 
其他非 0 值代表算法服务异常。 
RetCode的顺序和入参中 Images 或 Urls 的顺序一致。
        :rtype: list of int
    ...
    @property
    def SucIndexes(self):
        r"""加入成功的人脸索引。索引顺序和入参中 Images 或 Urls 的顺序一致。 
例， Urls 中 有 3 个 url，第二个 url 失败，则 SucIndexes 值为 [0,2] 。
    ...
    def SucFaceRects(self):
        r"""加入成功的人脸框位置。顺序和入参中 Images 或 Urls 的顺序一致。
...
不同算法模型版本对应的人脸识别算法不同，新版本的整体效果会优于旧版本，建议使用“3.0”版本。
    ...
    @property
    def GroupId(self):
        r"""人员库 ID，不可修改，不可重复。支持英文、数字、-%@#&_，长度限制64B。
        :rtype: str
        """
    ...
    def GroupExDescriptions(self):
        r"""人员库自定义描述字段，用于描述人员库中人员属性，该人员库下所有人员将拥有此描述字段。 
最多可以创建5个。 
每个自定义描述字段支持[1,30]个字符。 
在同一人员库中自定义描述字段不可重复。 
例： 设置某人员库“自定义描述字段”为["学号","工号","手机号"]， 
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def PersonId(self):
        r"""人员ID，单个腾讯云账号下不可修改，不可重复。支持英文、数字、-%@#&_，长度限制64B。
        :rtype: str
        """
        return self._PersonId
    ...
    def PersonExDescriptionInfos(self):
        r"""人员描述字段内容，key-value。[0，60]个字符，可修改，可重复。
...
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    def UniquePersonControl(self):
        r"""此参数用于控制判断 Image 或 Url 中图片包含的人脸，是否在人员库中已有疑似的同一人。 
如果判断为已有相同人在人员库中，则不会创建新的人员，返回疑似同一人的人员信息。 
如果判断没有，则完成创建人员。 
0: 不进行判断，无论是否有疑似同一人在库中均完成入库； 
1:较低的同一人判断要求（百一误识别率）； 
...
class DeletePersonFromGroupRequest(AbstractModel):
    r"""DeletePersonFromGroup请求参数结构体
...
class DeletePersonFromGroupResponse(AbstractModel):
    r"""DeletePersonFromGroup返回参数结构体
...
不同算法模型版本对应的人脸识别算法不同，新版本的整体效果会优于旧版本，建议使用“3.0”版本。
    ...
    @property
    def MaxFaceNum(self):
        r"""最多处理的人脸数目。默认值为1（仅检测图片中面积最大的那张人脸），最大值为120。 
此参数用于控制处理待检测图片中的人脸个数，值越小，处理速度越快。
        :rtype: int
        """
        return self._MaxFaceNum
...
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    def NeedFaceAttributes(self):
        r"""是否需要返回人脸属性信息（FaceAttributesInfo）。0 为不需要返回，1 为需要返回。默认为 0。 
非 1 值均视为不需要返回，此时 FaceAttributesInfo 不具备参考意义。  
最多返回面积最大的 5 张人脸属性信息，超过 5 张人脸（第 6 张及以后的人脸）的 FaceAttributesInfo 不具备参考意义。  
提取人脸属性信息较为耗时，如不需要人脸属性信息，建议关闭此项功能，加快人脸检测速度。
        :rtype: int
...
建议：人脸入库操作建议开启此功能。
    ...
    @property
    def FaceModelVersion(self):
        r"""人脸识别服务所用的算法模型版本。目前入参支持 “2.0”和“3.0“ 两个输入。  
2020年4月2日开始，默认为“3.0”，之前使用过本接口的账号若未填写本参数默认为“2.0”。 
不同算法模型版本对应的人脸识别算法不同，新版本的整体效果会优于旧版本，建议使用“3.0”版本。
        :rtype: str
        """
...
不同算法模型版本对应的人脸识别算法不同，新版本的整体效果会优于旧版本，建议使用“3.0”版本。
    ...
    @property
    def FaceInfos(self):
        r"""人脸信息列表。包含人脸坐标信息、属性信息（若需要）、质量分信息（若需要）。
...
不同算法模型版本对应的人脸识别算法不同，新版本的整体效果会优于旧版本，建议使用“3.0”版本。
    ...
    @property
    def Image(self):
        r"""图片 base64 数据，base64 编码后大小不可超过5M（图片的宽高比请接近3:4，不符合宽高比的图片返回的分值不具备参考意义）。
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
        :rtype: str
        """
        return self._Image
...
本字段只有FaceModelVersion为3.0时才具备参考意义。
    ...
    @property
    def Score(self):
        r"""活体打分，取值范围 [0,100]，分数一般落于[80, 100]区间内，0分也为常见值。推荐相大于 87 时可判断为活体。可根据具体场景自行调整阈值。
本字段当且仅当FaceModelVersion为2.0时才具备参考意义。
        :rtype: float
        """
        return self._Score
...
class FaceAttributesInfo(AbstractModel):
    r"""人脸属性信息，包含性别( gender )、年龄( age )、表情( expression )、
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def Gender(self):
        r"""性别[0~49]为女性，[50，100]为男性，越接近0和100表示置信度越高。NeedFaceAttributes 不为 1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
        :rtype: int
        """
        return self._Gender
    ...
    @property
    def Age(self):
        r"""年龄 [0~100]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
        :rtype: int
        """
        return self._Age
    ...
    @property
    def Expression(self):
        r"""微笑[0(normal，正常))~100(laugh，大笑)]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
        :rtype: int
        """
    ...
    @property
    def Glass(self):
        r"""是否有眼镜 [true,false]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
        :rtype: bool
        """
        return self._Glass
    ...
    @property
    def Pitch(self):
        r"""上下偏移[-30,30]，单位角度。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。 
建议：人脸入库选择[-10,10]的图片。
        :rtype: int
        """
        return self._Pitch
    ...
    @property
    def Yaw(self):
        r"""左右偏移[-30,30]，单位角度。 NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。 
建议：人脸入库选择[-10,10]的图片。
        :rtype: int
        """
        return self._Yaw
    ...
    @property
    def Roll(self):
        r"""平面旋转[-180,180]，单位角度。 NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。  
建议：人脸入库选择[-20,20]的图片。
        :rtype: int
        """
        return self._Roll
    ...
    @property
    def Beauty(self):
        r"""魅力[0~100]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
        :rtype: int
        """
        return self._Beauty
    ...
    @property
    def Hat(self):
        r"""是否有帽子 [true,false]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: bool
        """
        return self._Hat
    ...
    @property
    def Mask(self):
        r"""是否有口罩 [true,false]。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: bool
        """
        return self._Mask
    ...
    @property
    def Hair(self):
        r"""头发信息，包含头发长度（length）、有无刘海（bang）、头发颜色（color）。NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def EyeOpen(self):
        r"""双眼是否睁开 [true,false]。只要有超过一只眼睛闭眼，就返回false。 NeedFaceAttributes 不为1 或检测超过 5 张人脸时，此参数仍返回，但不具备参考意义。
注意：此字段可能返回 null，表示取不到有效值。
        :rtype: bool
        """
        return self._EyeOpen
...
class FaceDetailAttributesInfo(AbstractModel):
    r"""人脸属性信息，根据 FaceAttributesType 输入的类型，返回年龄（Age）、颜值（Beauty）
...
魅力( beauty )、眼镜( glass )、口罩（mask）、头发（hair）和姿态 (pitch，roll，yaw )。只有当 NeedFaceAttributes 设为 1 时才返回有效信息。
    ...
    def FaceQualityInfo(self):
        r"""人脸质量信息，包含质量分（score）、模糊分（sharpness）、光照分（brightness）、遮挡分（completeness）。只有当NeedFaceDetection设为1时才返回有效信息。
注意：此字段可能返回 null，表示取不到有效值。
...
class FaceQualityCompleteness(AbstractModel):
    r"""五官遮挡分，评价眉毛（Eyebrow）、眼睛（Eye）、鼻子（Nose）、脸颊（Cheek）、嘴巴（Mouth）、下巴（Chin）的被遮挡程度。
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def Score(self):
        r"""质量分: [0,100]，综合评价图像质量是否适合人脸识别，分数越高质量越好。 
正常情况，只需要使用Score作为质量分总体的判断标准即可。Sharpness、Brightness、Completeness等细项分仅供参考。
参考范围：[0,40]较差，[40,60] 一般，[60,80]较好，[80,100]很好。 
建议：人脸入库选取70以上的图片。
注意：此字段可能返回 null，表示取不到有效值。
...
class GetUpgradeGroupFaceModelVersionJobListRequest(AbstractModel):
    r"""GetUpgradeGroupFaceModelVersionJobList请求参数结构体
...
class GetUpgradeGroupFaceModelVersionJobListResponse(AbstractModel):
    r"""GetUpgradeGroupFaceModelVersionJobList返回参数结构体
...
class GetUpgradeGroupFaceModelVersionResultRequest(AbstractModel):
    r"""GetUpgradeGroupFaceModelVersionResult请求参数结构体
...
class GetUpgradeGroupFaceModelVersionResultResponse(AbstractModel):
    r"""GetUpgradeGroupFaceModelVersionResult返回参数结构体
...
有关更多信息，请参阅 Unix 时间。
    ...
    def EndTimestamp(self):
        r"""人员升级任务预估结束时间。 StartTimestamp的值是自 Unix 纪元时间到人员查重任务预估结束的毫秒数。  
Unix 纪元时间是 1970 年 1 月 1 日星期四，协调世界时 (UTC) 00:00:00。 
如果为0表示这个任务已经执行完毕。
        :rtype: int
        """
...
class ModifyPersonGroupInfoRequest(AbstractModel):
    r"""ModifyPersonGroupInfo请求参数结构体
...
class ModifyPersonGroupInfoResponse(AbstractModel):
    r"""ModifyPersonGroupInfo返回参数结构体
...
Unix 纪元时间是 1970 年 1 月 1 日星期四，协调世界时 (UTC) 00:00:00。有关更多信息，请参阅 Unix 时间。
    ...
    @property
    def CreationTimestamp(self):
        r"""人员的创建时间和日期 CreationTimestamp。CreationTimestamp 的值是自 Unix 纪元时间到Group创建时间的毫秒数。 
Unix 纪元时间是 1970 年 1 月 1 日星期四，协调世界时 (UTC) 00:00:00。有关更多信息，请参阅 Unix 时间。
        :rtype: int
        """
        return self._CreationTimestamp
...
class RevertGroupFaceModelVersionRequest(AbstractModel):
    r"""RevertGroupFaceModelVersion请求参数结构体
...
class RevertGroupFaceModelVersionResponse(AbstractModel):
    r"""RevertGroupFaceModelVersion返回参数结构体
...
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    @property
    def MaxFaceNum(self):
        r"""最多识别的人脸数目。默认值为1（仅检测图片中面积最大的那张人脸），最大值为10。 
MaxFaceNum用于，当输入的待识别图片包含多张人脸时，设定要搜索的人脸的数量。 
例：输入的Image或Url中的图片包含多张人脸，设MaxFaceNum=5，则会识别图片中面积最大的5张人脸。
        :rtype: int
        """
    ...
    @property
    def MinFaceSize(self):
        r"""人脸长和宽的最小尺寸，单位为像素。默认为34。低于34的人脸图片无法被识别。建议设置为80。
        :rtype: int
        """
        return self._MinFaceSize
    ...
    def MaxPersonNum(self):
        r"""单张被识别的人脸返回的最相似人员数量。默认值为5，最大值为100。 
例，设MaxFaceNum为1，MaxPersonNum为8，则返回Top8相似的人员信息。
值越大，需要处理的时间越长。建议不要超过10。
        :rtype: int
        """
    ...
    def NeedPersonInfo(self):
        r"""是否返回人员具体信息。0 为关闭，1 为开启。默认为 0。其他非0非1值默认为0
        :rtype: int
        """
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def FaceMatchThreshold(self):
        r"""出参Score中，只有超过FaceMatchThreshold值的结果才会返回。默认为0。
        :rtype: float
        """
        return self._FaceMatchThreshold
...
class SearchFacesReturnsByGroupRequest(AbstractModel):
    r"""SearchFacesReturnsByGroup请求参数结构体
...
例：输入的Image或Url中的图片包含多张人脸，设MaxFaceNum=5，则会识别图片中面积最大的5张人脸。
    ...
    @property
    def MinFaceSize(self):
        r"""人脸长和宽的最小尺寸，单位为像素。默认为34。低于34将影响搜索精度。建议设置为80。
        :rtype: int
        """
        return self._MinFaceSize
    ...
    def MaxPersonNumPerGroup(self):
        r"""被检测到的人脸，对应最多返回的最相似人员数目。默认值为5，最大值为10。  
例，设MaxFaceNum为3，MaxPersonNumPerGroup为5，GroupIds长度为3，则最多可能返回3*5*3=45个人员。
        :rtype: int
        """
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def FaceMatchThreshold(self):
        r"""出参Score中，只有大于等于FaceMatchThreshold值的结果才会返回。
默认为0。
取值范围[0.0,100.0) 。
        :rtype: float
        """
...
class SearchFacesReturnsByGroupResponse(AbstractModel):
    r"""SearchFacesReturnsByGroup返回参数结构体
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def FaceMatchThreshold(self):
        r"""出参Score中，只有大于等于FaceMatchThreshold值的结果才会返回。默认为0。取值范围[0.0,100.0) 。
        :rtype: float
        """
        return self._FaceMatchThreshold
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    @property
    def PersonNum(self):
        r"""搜索的人员库中包含的人员数。若输入图片中所有人脸均不符合质量要求，则返回0。
        :rtype: int
        """
        return self._PersonNum
...
class SearchPersonsReturnsByGroupRequest(AbstractModel):
    r"""SearchPersonsReturnsByGroup请求参数结构体
...
class SearchPersonsReturnsByGroupResponse(AbstractModel):
    r"""SearchPersonsReturnsByGroup返回参数结构体
...
class UpgradeGroupFaceModelVersionRequest(AbstractModel):
    r"""UpgradeGroupFaceModelVersion请求参数结构体
...
class UpgradeGroupFaceModelVersionResponse(AbstractModel):
    r"""UpgradeGroupFaceModelVersion返回参数结构体
...
若图片质量不满足要求，则返回结果中会提示图片质量检测不符要求。
    ...
    @property
    def PersonId(self):
        r"""待验证的人员ID。人员ID具体信息请参考人员库管理相关接口。
        :rtype: str
        """
        return self._PersonId
...
2.0版本误识率千分之一对应分数为70分，误识率万分之一对应分数为80分，误识率十万分之一对应分数为90分。 一般超过80分则可认定为同一人。
    ...
    @property
    def Score(self):
        r"""给定的人脸图片与 PersonId 对应人脸的相似度。若 PersonId 下有多张人脸（Face），返回相似度最大的分数。
...
2.0版本误识率千分之一对应分数为70分，误识率万分之一对应分数为80分，误识率十万分之一对应分数为90分。 一般超过80分则可认定为同一人。
    ...
    @property
    def IsMatch(self):
        r"""是否为同一人判断，固定阈值分数为60分，若想更灵活地调整阈值可取Score参数返回进行判断
        :rtype: bool
        """
...
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
    ...
    @property
    def Url(self):
        r"""图片的 Url 。 图片的 Url、Image必须提供一个，如果都提供，只使用 Url。 
图片存储于腾讯云的Url可保障更高下载速度和稳定性，建议图片存储于腾讯云。 
非腾讯云存储的Url速度和稳定性可能受一定影响。
若图片中包含多张人脸，只选取其中人脸面积最大的人脸。
支持PNG、JPG、JPEG、BMP，不支持 GIF 图片。
...
class VerifyPersonResponse(AbstractModel):
    ...
    @property
    def Score(self):
        r"""给定的人脸照片与 PersonId 对应的相似度。若 PersonId 下有多张人脸（Face），会融合多张人脸信息进行验证。
        :rtype: float
        """
        return self._Score
