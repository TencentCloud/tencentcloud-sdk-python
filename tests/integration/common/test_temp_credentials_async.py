...
from tencentcloud.sts.v20180813 import sts_client_async, models
...
async def _get_temp_redentials():
    try:
        ...
        client = sts_client_async.StsClient(cred, "ap-guangzhou", clientProfile)
        ...
        resp = await client.AssumeRole(req)
...
@pytest.mark.asyncio
async def test_temp_credential_ok():
    temp_info = await _get_temp_redentials()
    for reqmethod in reqmethod_list:
        ...
        clientProfile = ClientProfile()
        clientProfile.httpProfile = httpProfile
        client = cvm_client_async.CvmClient(temp_cred, "ap-guangzhou", clientProfile)
...
@pytest.mark.skipif(not os.environ.get("TENCENTCLOUD_ROLE_ARN"),
                    reason="TENCENTCLOUD_ROLE_ARN env var not found")
@pytest.mark.asyncio
async def test_temp_credential_invalid():
    temp_info = await _get_temp_redentials()
    for reqmethod in reqmethod_list:
        ...
        clientProfile = ClientProfile()
        clientProfile.httpProfile = httpProfile
        client = cvm_client_async.CvmClient(invalid_cred, "ap-guangzhou", clientProfile)
