...
setup(
    name='tencentcloud-sdk-python-%(mod)s',
    %(install_requires)s
    %(extras_require)s
    version=tencentcloud.__version__,
    description='Tencent Cloud %(mod_cap)s SDK for Python',
...
def mk_config_file(temp_dir, mod):
    if mod == 'common':
        install_requires = 'install_requires=["requests>=2.16.0"],'
        extras_require = 'extras_require={"async": ["httpx>=0.22.0"]},'
    else:
        install_requires = 'install_requires=["tencentcloud-sdk-python-common>=%s,<%s"],' % (__version__, highest_version)
        extras_require = ''
